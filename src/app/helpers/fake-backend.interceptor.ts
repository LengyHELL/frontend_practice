...
export class FakeBackendInterceptor implements HttpInterceptor {
  ...
  getUsers(request: HttpRequest<User>) : Observable<HttpEvent<User[]>> {
    if (!this.isLoggedIn(request)) throw new HttpHeaderResponse({status: 401, statusText: "Unauthorized"});
    return of(new HttpResponse<User[]>({status: 200, body: users}));
  }

  isLoggedIn(request: HttpRequest<User>) : boolean {
    return request.headers.get('Authorization') === `Basic ${window.btoa('test:test')}`
  }
}

export let fakeBackendProvider = { //fake backend for http service
